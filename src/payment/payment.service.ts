...
interface ExtendedSendMailOptions {
  to: string;
  subject: string;
  template: string;
  context: any;
        ...
        if (user.token === session.id) {
          ...
          let [, profile, response] = await Promise.all([
            this.userModel.findOneAndUpdate({ _id: user_id }, {
              job_credits: user.job_credits + Number(job_credits),
              ad_credits: user.ad_credits + Number(ad_credits),
              banner_ad_days: user.banner_ad_days + Number(banner_ad_days),
              landing_page_ad_days: user.landing_page_ad_days + Number(landing_page_ad_days),
              token: null
            }),
          ...
          await Promise.all([
            ...
            this.emailService.sendMail(<ExtendedSendMailOptions>{
              to: user.email,
